...
module.exports = {
	...
	async run(client, message, args) {
		...
		if (dose.startsWith("ch")) {
			let s = await client.db.get("chillc" + message.author.id);
				if (s) {
					let time = client.cooldown(s, Date.now(), ms('6h'));
			...
			await client.db.set(`chillc${message.author.id}`, Date.now(), ms("6h"));
		}
	}
}