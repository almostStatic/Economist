...
async run(client, message, args) {
	...
	if(!queryString){
			...
			message.channel.awaitMessages(filter, {
					...
					if (collected.first().content.toLowerCase() == 'cancel') {
							return message.reply(client.config.emoji.tick + ' Command cancelled')
					}
					...
					if (!info || !res) {
							message.reply(client.config.emoji.err + ' I can\'t fint the requiested information on the Discord.js Documentation!')
					}
	...
	if (!embed) {
			return message.reply(`${client.config.emoji.err} ${client.user.username} **couldn't** find the requested information.\nMaybe look for something that actually exists next time?`);
	};
